...
exports.register = async (req, res) => {
  ...
  try {
    ...
    let user = await Faculty.findOne({ email: userfields.email });
    if (!user) {
      user = new Faculty(userfields);
...
exports.userInfoUpdate = async (req, res) => {
  if (req.body.userId === req.params.id) {
    ...
    try {
      const updatedUser = await Faculty.findByIdAndUpdate(
        req.params.id,
        {
          $set: req.body,
        },
